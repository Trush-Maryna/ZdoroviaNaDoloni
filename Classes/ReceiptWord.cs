    ...
    public class ReceiptWord
    {
        public static object CreateReceiptSelfPickup(List<OrderBasket.Feedback> panelDataList, decimal totalPrice, int totalCount, List<MapManager.Pharmacy> pharmacyMarks)
        {
            try
            {
                ...
                AddTitle(doc);
                AddProductsTable(doc, panelDataList);
        ...
        private static void AddProductsTable(Document doc, List<OrderBasket.Feedback> panelDataList)
        {
            ...
            Table productTable = doc.Tables.Add(productsHeader.Range, panelDataList.Count + 1, 3);
            productTable.Borders.Enable = 1;

            productTable.Cell(1, 1).Range.Text = "№";
            productTable.Cell(1, 2).Range.Text = "Назва";
            productTable.Cell(1, 3).Range.Text = "Виробник";

            for (int i = 0; i < panelDataList.Count; i++)
            {
                var productData = panelDataList[i].Product;
                productTable.Cell(i + 2, 1).Range.Text = (i + 1).ToString();
                productTable.Cell(i + 2, 2).Range.Text = productData.Name;
                productTable.Cell(i + 2, 3).Range.Text = productData.Developer;
            }

            FormatTable(productTable);
        }
        ...
        private static void FormatTable(Table table)
        {
            ...
            table.Range.ParagraphFormat.SpaceAfter = 10;
        }
